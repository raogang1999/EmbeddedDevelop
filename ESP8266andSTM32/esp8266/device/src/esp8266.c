...
{ 
    ...
    delay_ms(time);   //延时
		...
		//需要接收数据，将其变为一个字符串;这里通过中断给他赋值了
    ESP8266_Fram_Record_Struct.Data_RX_BUF[ESP8266_Fram_Record_Struct.InfBit.FramLength ] = '\0';
		
		//把发送的命令打印出来
    printf("receive and send : %s",ESP8266_Fram_Record_Struct .Data_RX_BUF);
...
*/
bool ESP8266_MQTTCONNCFG(int linkId,int keepalive,int keepSession,char *lwt_topic,char *lwt_msg,int lwt_qos,int lwt_retain)
{
    char cCmd [120];
    sprintf ( cCmd,"AT+MQTTCONNCFG=0,%d,%d,\"%s\",\"%s\",%d,%d",keepalive,keepSession,lwt_topic,lwt_msg,lwt_qos,lwt_retain);
    return ESP8266_Send_AT_Cmd( cCmd, "OK", NULL, 500 );
}
